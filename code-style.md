...
1. Не создавайте временные переменные без причины. Переменные нужны только если имя добавляет смысл или значение переиспользуется
   ...
   ```
1. Старайтесь функции, классы и модули держать небольшими. 20-40 строк на функцию, 100 строк на класс, 200-300 символов на модуль — это максимум, но лучше делать эти сущности меньше
