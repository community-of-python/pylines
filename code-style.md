...
1. Необходимо писать самодокументируемый код:

   1. Имена всех переменных, функций, модулей, классов (любых идентификаторов и имен в коде, то есть) должны иметь длину не менее 8 символов. Имена типа `a`, `b` запрещены
...
1. Необходимо избегать магии, т.е. неявного поведения:
   1. hasattr, getattr — признаки такого поведения, но не только
   1. Магические числа, т.е. всё, что не является нулём или единицой, запрещены. Они должны быть выделены либо в глобальные settings.py, либо в константы модуля с семантичным названием. Это касается любых типов данных: чисел, строк, массивов и так далее
1. Не создавайте временные переменные без причины. Переменные нужны только если имя добавляет смысл или значение переиспользуется
   ...
   ```
1. Старайтесь функции, классы и модули держать небольшими. 20-40 строк на функцию, 100 строк на класс, 200-300 символов на модуль — это максимум, но лучше делать эти сущности меньшего размера
