...
def is_stdlib_package(module_name: str) -> bool:
    if not is_stdlib_module(module_name):
        return False
    spec: typing.Final = importlib.util.find_spec(module_name)
...
class COP002Check(ast.NodeVisitor):
    ...
    def _check_stdlib_import(self, node: ast.ImportFrom) -> None:
        if node.module == "__future__":
            return
        if (is_stdlib_module(node.module) and not is_stdlib_package(node.module)) or (
            "." in node.module and is_stdlib_package(node.module.split(".")[0])
        ):
            self.violations.append(Violation(node.lineno, node.col_offset, ViolationCode.STDLIB_IMPORT))
