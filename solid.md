...
> ⚠️ **Warning:** Гайд на ранних стадиях запуска, используйте аккуратно :)
...
Давайте рассмотрим более понятный пример. Представим, что мы разрабатываем систему для офисных устройств. У нас есть устройство "всё-в-одном", которое может печатать, сканировать и факсовать. Мы здесь не используем protocol для того, чтобы не «раздувать» код примера:
...
class AllInOneDevice:
    ...
    def scan_document(self):
        ...
        return {"content": "Содержимое отсканированного документа"}

    def send_fax(self, document, recipient: str) -> None:
        print(f"Отправляю факс '{document.content}' адресату {recipient}")

# Функция, которая хочет только сканировать и печатать
def copy_document(device) -> None:
...
class OfficeAllInOne:
    ...
    def send_fax(self, document, recipient: str):
        print(f"Отправляю факс '{document.content}' адресату {recipient}")

# Теперь функция copy_document зависит только от необходимых интерфейсов
def copy_document(scanner, printer):
...
# 2. Функция, которая использует факс (только те, кто реализуют этот интерфейс)
def send_document_via_fax(fax_machine, ...):
    ...

# Только OfficeAllInOne может отправлять факс, SimplePrinter и SimpleScanner не могут
send_document_via_fax(OfficeAllInOne(), ...)
```

Почему так лучше:
* Клиентская функция `copy_document` зависит только от необходимых интерфейсов (`scan_document` и `print_document`), а не от лишнего (`send_fax`)
