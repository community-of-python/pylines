...
* нет нужды следовать принципу фанатично. Т.е. если вы столкнулись переписыванием одного и того же кода, устранили проблему, но иногда небольшими кусками продолжаете его изменять, то в этом, вероятно, нет большой проблемы. Что такое «небольшими», на мой взгляд, стоит определить самостоятельно. Я бы оценивал как этот объём ~ до 10 строк, не впадая в крайности. Те, если строк 12 — это всё ещё небольшой объём
...
Давайте рассмотрим более понятный пример. Представим, что мы разрабатываем систему для офисных устройств. У нас есть устройство "всё-в-одном", которое может печатать, сканировать и факсовать:
...
# Проблемный подход - один класс со всеми возможностями
class AllInOneDevice:
    def print_document(self, document: Document) -> None:
        print(f"Печатаю документ: {document.content}")

    def scan_document(self) -> Document:
        print("Сканирую документ")
        return Document("Содержимое отсканированного документа")

    def send_fax(self, document: Document, recipient: str) -> None:
        print(f"Отправляю факс '{document.content}' адресату {recipient}")

# Функция, которая хочет только сканировать и печатать
def copy_document(device: AllInOneDevice) -> None:
    # Эта функция использует только 2 из 3 методов устройства
    document = device.scan_document()  # Используется
    device.print_document(document)    # Используется
    # device.send_fax(...) - этот метод не нужен, но уже реализован

# Использование
all_in_one = AllInOneDevice()
copy_document(all_in_one)
```

Проблема в том, что `copy_document` зависит от метода `send_fax`, который ей не нужен. Если мы захотим использовать устройство, которое умеет только сканировать и печатать (но не факсовать), мы не сможем это сделать.

Решение - разделить интерфейсы согласно принципу ISP:
...
# Разделяем интерфейсы на отдельные протоколы
...
# Теперь функция copy_document зависит только от необходимых интерфейсов
...
# Теперь мы можем использовать разные комбинации устройств
# 1. Раздельные устройства
scanner = SimpleScanner()
printer = SimplePrinter()
copy_document(scanner, printer)

# 2. Все-в-одном устройство (оно реализует оба интерфейса)
all_in_one = OfficeAllInOne()
copy_document(all_in_one, all_in_one)  # То же устройство как сканер и принтер

# 3. Функция, которая использует факс (только те, кто реализуют этот интерфейс)
def send_document_via_fax(fax_machine: FaxMachine, document: Document, recipient: str) -> None:
    fax_machine.send_fax(document, recipient)

# Только OfficeAllInOne может отправлять факс, SimplePrinter и SimpleScanner не могут
send_document_via_fax(all_in_one, Document("Документ для факса"), "Иван Иванов")
```

Почему так лучше:
* Клиентская функция `copy_document` зависит только от необходимых интерфейсов (`Scanner` и `Printer`), а не от лишнего (`FaxMachine`)
* Мы достигаем снижения связности - функция не знает и не зависит от методов, которые ей не нужны
* Гибкость - можно комбинировать разные устройства для разных задач
* Расширяемость - легко добавить новые типы устройств без изменения существующего кода
* Ясность - каждый интерфейс имеет четкое предназначение
