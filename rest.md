...
* каждый адрес — это ресурс или сущность. Что такое «ресурс» или «сущность»? Филдинг предлагает считать ресурсом любую сущность, не обязательно ту, которая лежит в базе, у него это может быть и результатом вычислений, абстрактным понятием и т.п. Мы трактуем это уже: для нас обычно это какая-то структура с фиксированными полями. Ну, например, можете воспринимать это как таблицу в БД или pydantic модель, или и то и другое. Это грубое описание, но достаточное.
...
* соглашения по построению адреса:
  * префикс /api/ не очень понятен, плюс оставляет не очень много пространства для маневра, поэтому мы используем следующие префиксы/layout:
    ...
    * вложенность запрещена. Нельзя делать так: /rest/\[имя-сервиса]/\[что-то]/\[что-то], /rpc/\[имя-сервиса]/\[что-то]/\[что-то]/. Схема построения урлов: /[тип протокола]/[имя-сервиса]/[что-то]/
...
* имена префиксов
  * /rest/\* — базовое REST API (админка, фронт, просто апи)
  ...
  * /rest-admin/\*, /rpc-admin/\* — админка (если ваше приложение не админка, то имеет смысл завести такие неймспейсы)
